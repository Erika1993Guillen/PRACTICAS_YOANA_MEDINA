...
x = valor cualqueira\\
p = probabilidad \\
lambda = media de la distribuci\'on\\
size = tama\~no de la muestra\\ 
\end{center}
...
Si un estudiante responde al azar a un examen de 8 preguntas de verdadero o falso.
 ...
 \end{description}
La variable X="n\'umero de aciertos" sigue una distribuci\'on Binomial de par\'ametros n = 8 y p=1$/$2(p probabilidad de \'exito).
<<>>=
...
@
 \end{description}
\begin{description}
   \item b) \¿Cu\'al es la probabilidad de que acierte 5 o m\'as? P[X$>$=5]
<<>>=
x <- 4; n=8; p=1/2  
...
\end{description}  
Para el primer participante la variable X="n\'umero de premios conseguidos entre 2 posibles" sigue una distribuci\'on hipergeom\'etrica de par\'ametros m=11, n=4, K=2. 
<<>>=
x <- 0:2; m = 11; n <- 4; k=2 

# x define el numero de globos con premio 
...
Calcula:
\begin{description}
  \item a) La probabilidad de que en un d\'ia determinado consiga vender la primera alarma en la sexta casa que visita. 
\end{description}
...
# creando la tabla de distribucion de frecuencias del numero de intentos 
# fallidos antes de obtener la primera venta. 

Tabla <- data.frame(Probabilidad=dgeom(x, prob=p))

# nombrando las filas de la distribucion de frecuencias
...
# Graficar la función de probabilidad teórica, use la función points(), no 
# debe cerrar el gráfico obtenido con la instrucción anterior
...
# generar 100 números aleatorios de la distribucion
...
xlab="X = Número de eventos a una tasa constante", ylab="masa de 
probabilidad", probability=TRUE, col="blue") 

# Graficar la funcion de probabilidad teorica, use la funcion points()
