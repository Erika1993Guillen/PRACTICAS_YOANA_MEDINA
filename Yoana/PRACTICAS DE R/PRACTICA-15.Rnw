...
# Graficar la funci\'on de densidad, use la funci\'on curve() para variable continua 
...
# density() y plot() para dibujar su 
"gráfica"
plot(density(x.norm), main="Densidad estimada de los datos")

# R permite calcular la funci\'on de distribuci\'on acumulada te\'orica con ecdf() 
...
<<>>=
# Definir el par\'ametro apropiado 
media <- 2500; razon <- 1/media;n=100 

# generar 100 números aleatorios de la distribuci\'on 
x = rexp(n, razon); x 
...
# Graficar la funci\'on de densidad, usando la funci\'on curve() 
