...
<<>>=
v <- numeric(3);v
# El vector tiene longitud 3 y sus componentes ser\'ian NA (datos omitidos o faltantes).
@
Ejemplo 2: 
<<>>=
v[3] <- 17; v
...
\\ Ejemplo 1:
<<>>=
x <- c(2, 4, 3.1, 8, 6);is.integer(x);is.double(x);length(x);x
@
...
<<>>=
y = 1:4; y
# Crea un vector de valores enteros en que su primer elemento es 1 su
# \'ultimo
#es 4
@
...
<<>>=
u <- 1:12; u1=u[2 * 1:5];u1
# Vector de tama\~no 5 con elementos de las posiciones pares de u
@
...
#elemento, el vector 2*x repetido 2.2 veces, el vector y, y el número 1
#repetido 11 veces "Reciclado en R es repetir las veces necesarias un vector
#cuando en una operaci\'on intervienen vectores de distinta longitud"
@
Ejemplo 3:
<<>>=
...
<<>>=
t(e1)%*%e2 
# Calcula el producto interno entre dos vectores. Ambos deben tener el 
...
Ejemplo 1:
<<>>=
resum <- c(length(y), sum(y), prod(y), min(y), max(y)); y;resum
@
...
names(codDeptos) <- c("Usulután", "San Miguel", "Moraz\'an", "La Uni\'on");codDeptos
Oriente <- codDeptos [c("La Uni\'on", "San Miguel")];Oriente
@
...
#dos índicespara indicar fila y columna.
@
...
x2 <- seq(1, 11, 2); x2
x3 <- runif(6); x3 
# Vector con valores de una uniforme(0,1)
@
...
<<>>=
T=sqrt(B); T
#Observe que la ra\'iz se saca a cada elemento de la matriz.
@
...
#sort() genera um vector en los cual sus elementos han sido ordenados de menor 
#a mayor a partir de los elementos de la matriz C
@
...
Ejemplo 1:
<<>>=
nombres <- matrix(c("Carlos", "José", "Ana", "Ren\'e", "Mar\'ia", "Mario"), 
